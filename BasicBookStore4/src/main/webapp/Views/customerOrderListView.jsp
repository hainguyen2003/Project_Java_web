...
<meta charset="UTF-8">
<link rel="styleseet"
	href="${pageContext.request.contextPath}/css/boostore_style.css">
				...
				<c:forEach items="${orderListOfCustomer}" var="orderOfCustomer">
					<tr>
						...
						<td>${orderOfCustomer.orderStatusDescription}
							<c:if test="${Constant.WARTING_CONFIRM_ORDER_STATUS != orderOfCustomer.orderStatus}">
								&nbsp;-
						...
						<td>
							...
							<div id ="div${orderOfCustomer.orderId}" style="display: none;">
								...
								<table>
									...
									<c:forEach items="${orderOfCustomer.orderBookList}" var="cartItem">
										<tr>
											...
											<td><fmt:formatNumber type="number" maxFractionDigits="0" value="${cartItem.selectedBook.price}"></fmt:formatNumber><sup>đ</sup></td>/-strong/-heart:>:o:-((:-h<td>${cartItem.quantity}</td>
								...
								Tổng số tiền:
								<b>
									<span> <fmt:formatNumber type="number" maxFractionDigits="0" value=""></fmt:formatNumber>										
									 </span>
									 <sup>đ</sup>
								</b>
							</div>
